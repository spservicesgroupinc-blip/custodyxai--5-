...
const MessageAnalyzer: React.FC<{ documents: StoredDocument[]; userProfile: UserProfile | null; isOffline: boolean }> = ({ documents, userProfile, isOffline }) => {
...
const PatternAnalysis: React.FC<PatternAnalysisProps> = ({ reports, documents, userProfile, isOffline }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            {mode === 'reports' ? (
                ...
                <MessageAnalyzer documents={documents} userProfile={userProfile} isOffline={isOffline} />
            )}
        </div>
    );
};
